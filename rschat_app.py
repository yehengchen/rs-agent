    ...
    def inference(self, inputs):
        ...
        if viz:
            ...
            if show.lower() == 'yes':
                ...
                print("Please check the image in the output folder.")
        else:
            print("Please check the image in the output folder.")
...
class ObjectCounting:
    ...
    def __init__(self, device):
        if img_ch == '1':
            self.func = CountingFuncnction_ship(device)
        else:
    ...
    @prompts(name="Count object",
             description="useful when you want to count the number of the object in the image. "
                         "like: how many ships are there in the image or singapore? or count the number of ships"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_dir', type=str, default='/home/mars/cyh_ws/LLM/Remote-Sensing-Chat/image/rs-ship.jpg', required=False)
    ...
    parser.add_argument('--img_ch', type=str, default='3',choices=['1','3'], help='1 for PAN/Gray, 3 for MS/RGB')
    parser.add_argument('--viz', type=bool, default= False ,choices=[True, False])
    parser.add_argument('--chat_type', type=str, default='single', choices=['single','multi'], help='single-turn conversations or muli-turn conversations')
    ...
    img_ch = args.img_ch
    viz = args.viz
    conversations_type = args.chat_type

    count_num = ""
    state = []
    load_dict = {e.split('_')[0].strip(): e.split('_')[1].strip() for e in args.load.split(',')}
    bot = RSChat(gpt_name=args.gpt_name, load_dict=load_dict, openai_key=args.openai_key, proxy_url=args.proxy_url)
    bot.initialize(args.language)
    print('RSChat initialization done, you can now chat with RSChat~')
    
    # single-turn conversations
    if conversations_type == 'single':
        ...
        # txt = '之江实验室附近有农田吗？请用工具绘制农业用地区域'

        # txt = 'Is there basketball court near Zhejiang Lab? Please detect the basketball court'
        # txt = 'Is there bridge near Zhejiang Lab? Please detect the bridge'
    ...
    # muli-turn conversations
    if conversations_type == 'multi':
        while True:
