...
DEVICE = "cuda"  # 使用CUDA
DEVICE_ID = "0,1"  # CUDA设备ID，如果未设置则为空
...
class Qwen2_LLM(LLM):
    ...
    def _call(self, prompt : str, stop: Optional[List[str]] = None,
        ...
        input_ids = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, top_p=0.8, temperature=0.0001, repetition_penalty=1.1)
...
class Qwen2_5_LLM(LLM):
    ...
    def _call(self, prompt : str, stop: Optional[List[str]] = None,
        ...
        input_ids = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, top_p=0.8, temperature=0.0001, repetition_penalty=1.1)
        model_inputs = self.tokenizer([input_ids], return_tensors="pt").to('cuda')
        generated_ids = self.model.generate(model_inputs.input_ids, attention_mask=model_inputs['attention_mask'], max_new_tokens=512,use_cache=True)
        generated_ids = [
            output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated_ids)
        ]
        response = self.tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
